...
bool MLIRStatsAnalysis::runAnalysis(FuncOp op) {
  // Ignore functions generated by affine optimization which have names S0, S1,
  // ...
  // TODO this is a terrible way to do this but it works
  if (op.getName().str().size() == 2) {
    return true;
  }

  auto bb = analyzeBasicBlocks(op);
  auto instr = analyzeInstrutions(op);
  return print_stats(bb, instr);
}
